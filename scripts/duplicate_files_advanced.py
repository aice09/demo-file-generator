...
filename = (
f"{uuid.uuid4().hex}{ext}" if randomize else f"{name}_{i}{ext}"
)
...
list(tqdm(executor.map(copy_task, tasks), total=len(tasks), desc="Generating files"))
...
print("SUCCESS")
print(f"Files prepared : {total_files}")
if zip_path:
